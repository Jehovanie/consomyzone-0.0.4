/*
    import { settingDateToStringMonthDayAndYear } from "./../function.js"
 */
class ResponseAvis {
	...
	reagirAvis(elem) {
		...
		let nb_reaction = document.querySelector(".avis_elie_js_" + id)
			? document.querySelector(".avis_elie_js_" + id).textContent
			: 0;

		/** Post reaction from user */
		fetch("/" + this.rubrique + "/avis/push-reaction/" + id)
			.then((d) => d.json())
			.then((d) => {
				if (d.result == "success") {
	...
	sendResponseAvis(elem) {
		...
		fetch("/" + this.rubrique + "/avis/push-response", {
			...
			.then((d) => {
				...
				let user = d.user;
				if (d.result == "success") {
					document.querySelector(".avis_response_elie_js_" + id_avis).textContent =
						parseInt(nb_reponse) + 1 + " réponse(s)";
					// console.log("inserted");

					let new_li = document.createElement("li");
					new_li.classList = "card card-body";
					let my_photo = user.photoProfil
						? "/public" + user.photoProfil
                    ...
                    <div>
                        <span title="J'aime" data-id="${
							d.last_id
						}" class="badge rounded-pill text-bg-light cursor-pointer " onclick="reagirResponse(this)"><i class="fa-solid fa-thumbs-up"></i> J'aime</span>
                        . <i class="fa-regular fa-thumbs-up "></i> <small class=" avis_elie_js_${d.last_id}"> 0</small>
                        |
                        <span class="badge rounded-pill text-bg-light cursor-pointer" data-bs-toggle="collapse" data-bs-target="#collapseResponseInput2${
							d.last_id
						}"><i class="fa-solid fa-reply"></i> Répondre</span>
                        . <span class="cursor-pointer" data-bs-toggle="collapse" data-bs-target="#subresponseData_${
							d.last_id
						}"><i class="fa-solid fa-reply-all text-secondary"></i> <small class="text-primary avis_response_elie_js_${
						d.last_id
					}"> 0 réponse(s)</small></span>
                        <div class="collapse" id="collapseResponseInput2${d.last_id}">
                            <div class="d-flex mt-2 input-group">
                                <input class="form-control" id="input_response_2_${
									d.last_id
								}" placeholder="Ajouter une réponse...">
                                <button data-id="${
									d.last_id
								}" class="btn btn-outline-primary btn-sm" onclick="sendResponseForResponseAvis(this)"><i class="fa-solid fa-paper-plane"></i></button>
                            </div>
                        </div>
                    </div>
                `;
					...
					document.querySelector("#collapseResponseData" + id_avis + " > h5").textContent =
						new_nb_resp + `${new_nb_resp > 1 ? " Réponses" : " Réponse"}`;
	...
	showMinusResponseAvis(elem) {
		...
		let content =
			nexts_li_d_none.length > 1 ? "Voir les " + nexts_li_d_none.length + " réponses" : "Voir l'une réponse";
	...
	reagirSubResponse(elem) {
		let id = elem.getAttribute("data-id");
		let nb_reaction = document.querySelector(".avis_elie_js_2_" + id)
			? document.querySelector(".avis_elie_js_2_" + id).textContent
			: 0;

		/** Post reaction from user */
		fetch("/" + this.rubrique + "/avis/push-reaction-subresponse/" + id)
			.then((d) => d.json())
			.then((d) => {
				// console.log(d);
				if (d.result == "success") {
			...
			.then((d) => {
				...
				if (d.result == "success") {
                    ...
                    <div>
                        ...
                        . <i class="fa-regular fa-thumbs-up "></i> <small class=" avis_elie_js_2_${
							d.last_id.id
						}"> 0</small>
                        |
                        <span class="badge rounded-pill text-bg-light cursor-pointer" data-bs-toggle="collapse" data-bs-target="#collapseResponseInput2${
	...
	showResponseAvis(responses) {
		...
		if (responses.length > 0) {
			for (let reponse of responses) {
				let ul = `<ul class="collapse" id="subresponseData_${
					reponse.id
				}"><h5 class="badge rounded-pill text-bg-light">${reponse.all_response.length} ${
					reponse.all_response.length > 1 ? " Réponses" : " Réponse"
				}</h5>`;
				let li = "";
			...
			<li class="card card-body">
				<div class="d-flex align-items-center">
					<div class="content_profil_image me-2">
						<img class="profil_image_v2" src="/public${
							reponse.photo_profil ? reponse.photo_profil : "/uploads/users/photos/default_pdp.png"
						}" alt="User">
					</div>
					<div class="content_info">
				...
				<div>
					<span title="${title_react_reponse}" data-id="${
					reponse.id
				}" class="badge rounded-pill text-bg-light cursor-pointer ${col_react_reponse}" onclick="reagirResponse(this)"><i class="fa-solid fa-thumbs-up"></i> J'aime</span>
					${nb_reaction_reponse}
					|
					<span class="badge rounded-pill text-bg-light cursor-pointer" data-bs-toggle="collapse" data-bs-target="#collapseResponseInput2${
						reponse.id
