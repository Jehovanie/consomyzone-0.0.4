...
/**
 * @outhor Jehovanie RAMANDRIJOEL <jehovanieram@gmail.com>
 *
 * User for sending new letter ( tribuT/myTribuT.js )
 * @param {*} tribuTName
 */
function bindActionNewsLetterTribuT(tribuTName) {
	if (document.querySelector("#fetch_new_letter_fans_tribuT_jheo_js")) {
		const callActionNewLetterFans = document.querySelector("#fetch_new_letter_fans_tribuT_jheo_js");
...
function handleSubmitSendNewsLetter(tribuTName) {
...
function sendNewLetter(data) {
	//////fetch data
	fetch("/tributT/sendNewLetter/for_all", {
		method: "POST",
		headers: {
			Accept: "application/json",
			"Content-Type": "application/json",
		},
		body: JSON.stringify(data), /// object, description, piece_joint, table_name
	})
		.then((response) => response.json())
		.then((result) => {
			new swal("Succès !", "Lettre d'information envoyée.", "success").then((value) => {
				const btn_submit = document.querySelector(".btn_submit_sendNewsLetter_jheo_js");
				btn_submit.removeAttribute("disabled");
				btn_submit.textContent = "Envoyer";
