...
function renderMyTribu() {
	...
	if (params.size > 0) {
		...
		const id_c_u = document.querySelector(".information_user_conected_jheo_js")
			? document.querySelector(".information_user_conected_jheo_js").dataset.toggleUserId
			: 0;
...
async function showBlockPubV2(type, tribu_t_name, id_c_u) {
	let currentTribuSelected = document.querySelector(`a[data-table-name="${tribu_t_name}"]`);
				...
				fetch(request).then((responses) => {
					if (responses.ok && responses.status === 200) {
						...
						document.querySelector(
							"#activeTribu"
						).parentElement.parentElement.previousElementSibling.children[0].src = evt.target.result;
						swal({
							title: "Succès!",
...
function showdDataContent(dataFirst, type, tribu_t_name, id_c_u, lastId = 0) {
	...
	let tribu_t = Array.isArray(JSON.parse(detailsTribuT).tribu_t)
		? Array.from(JSON.parse(detailsTribuT).tribu_t).filter((e) => e.name == tribu_t_name)
	...
	if (lastId == 0) {
		...
		if (document.querySelector("#activeTribu")) {
			...
			canChangeTribuPicture = !document.querySelector("#activeTribu").classList.contains("other")
		...
		let canUpdateTribuInfo = !document.querySelector("#activeTribu").classList.contains("other")
			? `<li class="listNavBarTribu">
                                <a style="cursor:pointer;" id="settingTribuT" onclick="settingTribuT(event,'${tribu_t[0].name}')">Paramètre</a>
                            </li>`
			: "";

		let canUpdateTribuInfoMob = !document.querySelector("#activeTribu").classList.contains("other")
			? `<li class="listNavBarTribu">
                ...
                <div class="content-couverture mt-3">
                    <div class="row content-tribu-t">
                        ...
                        <div class="col-lg-8 col-8 content-tribu-t-name">
                            <h1 class="titre-tribu-t" id="tribu_t_name_main_head" data-tribu="${tribu_t[0].name}">${
			tribu_t[0].name_tribu_t_muable
				? tribu_t[0].name_tribu_t_muable
				: tribu_t[0].name.replace(/tribu_t_[0-9]+_/, "").replaceAll("_", " ")
		}</h1>
                            <p class="text-white descrp-tribu-t">
                            ${tribu_t[0].description ? tribu_t[0].description?.replace(/"/gi, "") : ""}
                            </p>
                        </div>
                    </div>
                ...
                <div class="container-fluid" style="height: 30px; background-color: #1ABA12;">
                     <p class="text-light">Tribu-t fondée par <span class="fw-bold">${dataFirst.pseudo}</span></p>
                </div>
                ${navMenuTribuT}
            </div>
					...
					data[i].id
                                            ...
                                            <div class="yd uf 2xl:ud-max-w-230-tribu-t rh ni bj wr nj xr content-pub">
                                                ...
                                                <div class="card-pub-actu">
                                                    <p class="text-pub"> ${data[i].publication
														.replace(/"/gi, "")
														.replace(/\\n/g, "</br>")}</p>
                                                    ${pub_photo}
                                                </div>
					...
					data[i].user_id
                                                    ...
                                                    <div class="reaction-icon d-flex">
                                                        <i style="cursor:pointer;" class="${
															data[i].reaction == 1
																? __userLoggedId == data[i].user_id_react
																	? "bi-heart-fill"
																	: "bi-heart"
																: "bi-heart"
...
<div class="publiate_on responsif-none-pc"><p  class="p-title"><span>${table_tribu_T_name}</span></p>
                                                        ...
                                                        <div class="status-content d-flex">
                                                            <p class="p-heure"> ${dataG.datetime}</p>
                                                            <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
                                                ...
                                                <div class="card-pub-actu">
                                                    <p class="text-pub"> ${dataG.publication.replace(/"/gi, "")}</p>
                                                    ${pub_photo}
                                                </div>
...
function showCommentaireTribu_T(event, idmin = 0, b) {
	...
	else if (document.querySelector("a.voir-plus")) event.target.parentNode.removeChild(event.target);
	...
	fetch(request).then((response) => {
		if (response.status === 200 && response.ok) {
			response.json().then((json) => {
				...
				else document.getElementById("center-content-cmnt" + json.pubId).appendChild(div);
			});
		}
	});
...
function showResto(table_rst_pastilled, id_c_u) {
	...
	if (document.getElementById("list_resto_pastilled")) {
		...
		for (let child of childreen) document.getElementById("list_resto_pastilled").removeChild(child);
	}
...
function printNodeGlobale(element, globalNote) {
	...
	for (let star of element) {
		...
		if (rankRange.includes(parseInt(star.dataset.rank, 10))) {
			if (parseInt(star.dataset.rank, 10) <= Math.trunc(globalNote)) star.style.color = "#F5D165";
			if (globalNote % 1 != 0) {
	...
	fetch(request).then((response) => {
		if (response.status == 200 && response.ok) {
			...
			let last_avis = parseInt(
				document.querySelector(".data-avis-" + _idResto).textContent.replaceAll(/[^0-9]/g, "")
			);
			...
			document
				...
				.setAttribute("onclick", "openAvis(" + parseInt(last_avis + 1) + "," + _idResto + ")");
			...
			document
				...
				.setAttribute(
					"onclick",
					openPopup.replaceAll("create", "update").replaceAll("Notez", "Modifier votre avis")
				);
...
function showActualites() {
	...
	const id_c_u = document.querySelector(".information_user_conected_jheo_js")
		? document.querySelector(".information_user_conected_jheo_js").dataset.toggleUserId
		: 0;
	if (id_c_u > 0) showBlockPubV2(type, tributName, id_c_u);
}
...
<h6 class="modal-title text-primary" >Vous pouvez modifier le corps de l'e-mail comme vous le voulez.</h6>
	...
	document
		...
		.setAttribute("data-table", document.querySelector("#tribu_t_name_main_head").dataset.tribu);
...
function setActiveTab(elem, param) {
	...
	switch (param) {
		case "tribu_g": {
			...
			document.querySelector(
				"#" + elem.parentElement.nextElementSibling.firstElementChild.dataset.element
			).style.display = "none";
			break;
		}
		case "email": {
			...
			document.querySelector(
				"#" + elem.parentElement.previousElementSibling.firstElementChild.dataset.element
			).style.display = "none";
...
function removePublication() {
	...
	fetch(
		new Request("/user/tribu/publication/remove/" + dataName + "_publication/" + id, {
			method: "GET",
			headers: {
				Accept: "application/json",
				"Content-Type": "application/json",
...
function updatePublication() {
	...
	let publicVisibilityElement = document.querySelector("#" + dataId).querySelector("[aria-labelledby=btnGroupDrop1]")
		.children[0];
	let privateVisibilityElement = document.querySelector("#" + dataId).querySelector("[aria-labelledby=btnGroupDrop1]")
		.children[1];
...
function openOnEvent(id, nom, adresse, action = "") {
	...
	else if (action == "resto") document.getElementById("restoRadio").checked = true;
...
function openPopupAction(id_pastille, denomination_f, adresse, text1, action, rubrique_type = "resto") {
	...
	switch (rubrique_type) {
		case "resto": {
			...
			document.getElementById("data-note-elie-js").innerHTML = `<i class="fas fa-edit"></i> ` + text1;
			document
				.getElementById("data-event-elie-js")
				.setAttribute("onclick", `openOnEvent(${id_pastille},'${denomination_f}','${adresse}',"resto")`);
			...
			if (document.querySelector(".send_avis_jheo_js")) {
				...
				document
					.querySelector(".send_avis_jheo_js")
					.setAttribute("onclick", `addAvisInTribuT("${id_pastille}","${rubrique_type}")`);
			}
			break;
		}
		case "golf": {
			...
			document
				...
				.setAttribute("onclick", `openOnEvent(${id_pastille},'${denomination_f}','${adresse}',"golf")`);
			const depastilledBtn = document.getElementById("data-depastilleGolf-nanta-js");
			depastilledBtn.dataset.id = id_pastille;
			depastilledBtn.dataset.name = denomination_f;
			depastilledBtn.dataset.tbname = document.querySelector("#activeTribu").getAttribute("data-table-name");
			break;
		}
	}
}
	...
	fetch("/api/agenda/restaurant/" + nom_dep + "/" + id_dep + "/detail/" + id_restaurant)
        ...
        `;
			...
			if (document.querySelector(".content_tow_cta >.site_web")) {
				document.querySelector(".content_tow_cta >.site_web").classList.toggle("non_active");
			}
		});
}
...
function settingTribuT(e, tribuTName) {
	...
	data.then((response) => {
		...
		document.querySelector("#updateTribuTName").value = currentTribuT.name_tribu_t_muable
			...
			: currentTribuT.name.replace(/tribu_t_[0-9]+_/, "").replaceAll("_", " "); //currentTribuT.name.replace(/tribu_t_[0-9]+_/, "").replaceAll("_", " ")
			...
			swal({
				...
				if (photoName != "") {
					document
						...
						.parentElement.parentElement.previousElementSibling.querySelector("img").src = base64;
				}
...
function findGolf(val, localisation = "") {
	...
	fetch(request)
		...
		.then((data) => {
			...
			jsons.length > 1
				? (document.querySelector("#extModalLabel").innerText = jsons.length + " golfs trouvés")
				: (document.querySelector("#extModalLabel").innerText = jsons.length + " golf trouvé");
 ...
 * @param {*} type
 */
function openPopupActionGolf(id_pastille = null, denomination_f = null, adresse = null, note, create, type) {
	...
	document
		...
		.setAttribute("onclick", `openOnEvent(${id_pastille},'${denomination_f}','${adresse}')`);
			...
			if (response.length > 0) {
				...
				for (const item of response) {
					...
					tbody_hist.innerHTML += `<tr>
                            ...
                            <td class="">${
								item.user
									? `<a href="/user/profil/${item.user.userId.id}" class="badge text-bg-primary">${
...
function fetchAllGalereInAlbum(idAlbum) {
	if (document.querySelector(".content-gal-all")) {
		document.querySelectorAll(".content-gal-all").forEach((item) => item.remove());
	}
	const requete = new Request("/user/tribu/photos/show/album/" + tribu_t_name_0 + "_publication", {
		method: "GET",
		headers: {
			Accept: "application/json",
			"Content-Type": "application/json",
	...
	fetch(requete)
		...
		.then((data) => {
			...
			document.querySelector(".all-galeri-to-album-tomm-js").innerHTML += li_img;
...
function copyePathAlbumTribuT(idAlbum) {
	const valuePathAlbum = document.querySelectorAll(".value-checked-album-tomm-js");
	...
	const request = new Request(`/user/tribu/photos/copyer/album/${tribu_t_name_0}`, {
		method: "POST",
		headers: {
			Accept: "application/json",
			"Content-Type": "application/json",
...
function getAlbumTribuT(tribu_t_types) {
	if (document.querySelector(".insert-album-tomm-js")) {
		document.querySelectorAll(".doc-album-tomm-js").forEach((item) => item.remove());
	}
	...
	fetch(requete)
		...
		.then((data) => {
          ...
          <div class="doc-album col-4 mb-2 album-${album.id}-tomm-js doc-album-tomm-js" onclick="docAlbumClick(${
					album.id
				}, '${album.name_album.replace("'", "").replace('"', "")}', '${tribu_t_types}')">
...
function getCopyePathAlbumTribuT(idAlbum) {
	const requete = new Request(`/user/tribu/get/copyer/album/${tribu_t_name_0}`, {
		method: "GET",
		headers: {
			Accept: "application/json",
			"Content-Type": "application/json",
	...
	fetch(requete)
		.then((response) => response.json())
		.then((data) => {
			let contentPhotoAlbum = document.querySelector(".insert-photo-album-tomm-js");
...
function createPostulantUI(postulants, container, tribuName) {
	...
	container.innerHTML = "";
	if (!document.querySelector("#postulant_modal")) createModalForPostulant(container);
	...
	postulants.forEach((postulant) => {
		...
		tdSelected.innerHTML = `<input class="slct_pstl_fan_js" type="checkbox" name="SelectedMe" data-loop="${cryptageJs(
			idTmp
		)}"/>`;
...
function relanceManyInvitation(tribuName) {
	const allCheckBox = Array.from(document.getElementsByClassName("slct_pstl_fan_js")).filter(
		(e) => e.checked === true
	);
...
function createBtnRelanceAll() {
	...
	btnRelanceAll.setAttribute("class", "btn btn-secondary btn_relance_all_faniry_js");
...
function listenCheckBoxPostulant(element1, elements) {
	elements.forEach((element) => {
		element.onchange = (e) => {
			const btnRelanceAll = document.querySelector(".btn_relance_all_faniry_js");
			const whoIsCheked = elements.filter((element2) => element2.checked === true);
...
function createModalForPostulant(container) {
	...
	document.getElementById("objet_area_pst").value = "Relance pour valider votre profil sur consomyzone.";
}
...
function sendEmailForPostulant(ObjetMail, mailContent, pieceJointe, idUserToSendEmail, element) {
	const data = {
		objetMail: ObjetMail,
 ...
 * @param {*} pieceJointe
 */
function sendEmailForManyPostulant(objetMail, mailContent, pieceJointe) {
	const data = {
