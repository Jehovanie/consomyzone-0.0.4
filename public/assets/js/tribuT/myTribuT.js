...
//// FUNCTION ADD EVENT ON SELECT OR CHANGE TRIBU T (this use list tribut T on right)
function showBlockPub() {
	...
	for (let array of arrays) {
		array.onclick = async (e) => {
			...
			updateStateUrl(id_c_u, tribu_t_name, type);
			...
			await showBlockPubItem(id_c_u, type, tribu_t_name);
		};
	}
}

////FETCH AND CHANGE TO ONE TRIBUT
async function showBlockPubItem(id_current_user, isOwnedOrJoined, tribut_table_name) {
	...
	// <a href="" class="tribu_t owned" data-tribu-rank="19" data-table-name="tribu_t_19_tribu_t_misy_resto">
	// 	Tribu T tribu T misy resto
	// </a>;

	const id_c_u = id_current_user;
	const type = isOwnedOrJoined;
	const tribu_t_name = tribut_table_name; ///data-table-name
...
/// BIND EVENT FONCTION ON ALL LIST TRIBU T
showBlockPub();
...
if (document.querySelector("#submit-publication-tribu-t")) {
	...
	if (document.querySelector("#publication_photo")) {
...
function showdDataContent(dataFirst, type, tribu_t_name, id_c_u, lastId = 0) {
	...
	if (type === "owned") {
		detailsTribuT = dataFirst.tribu_t_owned;
	} else {
		detailsTribuT = dataFirst.tribu_t_joined;
	}

	console.log(tribu_t_name);

	const data_tribuT = JSON.parse(detailsTribuT);
	console.log(data_tribuT);

	let tribu_t = Array.isArray(data_tribuT.tribu_t)
		? Array.from(data_tribuT.tribu_t).filter((e) => e.name == tribu_t_name)
		: [data_tribuT.tribu_t];
...
function openPopupAction(id_pastille, denomination_f, adresse, text1, action, rubrique_type = "resto") {
	...
	document
		...
		.setAttribute(
			"onclick",
			"openOnEvent(" + id_pastille + ",'" + denomination_f + "','" + adresse + "','" + action + "')"
		);
...
window.addEventListener("load", (event) => {
	if (document.querySelector(".content_chargment_tribut_jheo_js")) {
		const parsedUrl = new URL(window.location.href);
		const isOwnedOrJoin = parsedUrl.searchParams.get("nature");
		const tribuRankID = parsedUrl.searchParams.get("tribuRankID");
		const tribut = parsedUrl.searchParams.get("tribut");

		if (document.querySelector(`#${tribut}_jheo_js`)) {
			const item = document.querySelector(`#${tribut}_jheo_js`);

			item.id = "activeTribu";
			item.classList.add("p-2");
			item.classList.add("list-nav-left");
			item.classList.add("active"); //p-2 list-nav-left active

			showBlockPubItem(tribuRankID, isOwnedOrJoin, tribut);
		}
	}
});

function updateStateUrl(id_c_u, tribu_t_name, type) {
	const new_path = `/user/tribu/my-tribu-t?tribut=${tribu_t_name}&nature=${type}&tribuRankID=${id_c_u}`;
