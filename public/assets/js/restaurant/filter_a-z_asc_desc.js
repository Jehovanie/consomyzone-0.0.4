...
function createPagination(totalPages, page){
    ...
    if(page > 1){ //show the next button if the page value is greater than 1
        liTag += `<li class=" prev" onclick="createPagination(totalPages, ${page - 1})"><span><i class="fas fa-angle-left"></i></span></li>`;
    }
    ...
    for (var plength = beforePage; plength <= afterPage; plength++) {
        ...
        if(page == plength){ //if page is equal to plength than assign active string in the active variable
            if(OBJECT_MARKERS_RESTO.getAlreadyInit()){
                active = "alphabet_active";
                handleFilterFirstChar(letters[plength]);
                OBJECT_MARKERS_RESTO.filterByFirstLetterOnName(letters[page])
            }
    ...
    OBJECT_MARKERS_RESTO.setAlreadyInit(true)
...
function reverseCardElement(){
    const all_card_elements=document.querySelectorAll(".content_list_resto_spec_js_jheo .card-list.name_resto_js_jheo");
    document.querySelector(".content_list_resto_spec_js_jheo").innerHTML = "";
    for(let i= all_card_elements.length- 1 ; i>=0 ; i-- ){
        document.querySelector(".content_list_resto_spec_js_jheo").appendChild(all_card_elements[i]);
    }
}
...
function handleFilterFirstChar(letter){
    ...
    for(let i=0 ; i<all_card_elements.length ; i++ ){
        if(all_card_elements[i].querySelector(".id_departement_js_jheo").innerText.charAt(0).toLowerCase() !== letter.toLowerCase()){
...
function refreshDataList(){
    ...
    OBJECT_MARKERS_RESTO.resetToDefaultMarkers();
}