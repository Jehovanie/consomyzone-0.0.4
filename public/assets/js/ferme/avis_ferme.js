window.addEventListener('load', () => {
    const idFerme = location.href.toString().split("/")[8]
    ...
    if (document.querySelector(".FtBjOlVf") != null)
         showModifAreaFerme(idFerme,currentUserId)
    //body > main > div.content_global > div > div.content_home > div.left_content_home > div > div > div.content_note > div.nombre_avis
    if (document.querySelector("body > main > div.content_global > div > div.content_home > div.left_content_home > div > div > div.content_note > div.nombre_avis")) {
        showNemberOfAvisFerme(idFerme, document.querySelector("body > main > div.content_global > div > div.content_home > div.left_content_home > div > div > div.content_note > div.nombre_avis"))
        showNoteGlobaleFerme(idFerme)
    }
    ...
    document.querySelector("#text-note").onkeyup = (e) => { 
        ...
        mustBeInferior4Ferme(value, e.target)
    ...
    document.querySelector("#Submit-Avis-ferme-tom-js").onclick = () => {
       
        let note = document.querySelector("#text-note").value
        note=note.replace(/,/g,".")
        const avis = document.querySelector("#message-text").value
        try {
            mustBeInferior4Ferme(note, document.querySelector("#text-note"), true)  
            const requestParam = {
                note: parseFloat(note),
                avis:avis
            }
            //const idFerme=location.href.toString().split("/")[8]
            const request = new Request(`/avis/ferme/${idFerme}`, {
                method: "POST",
                headers: {
    ...
    if(document.querySelector("#see-tom-js")){
        document.querySelector("#see-tom-js").onclick = () => {
            ...
            showAvisFerme(currentUserId,idFerme) 
        }
    }
    ...
    if (document.querySelector("#UpDate-Avis-tom-js")) {
        document.querySelector("#UpDate-Avis-tom-js").onclick = () => { 
            ...
            console.log(avis)
            try {
                    mustBeInferior4Ferme(note, document.querySelector("#text-note-modif"), true)  
                    const requestParam = {
                        note: parseFloat(note),
                        avis:avis
                    }
                    ...
                    //const idFerme=location.href.toString().split("/")[8]
                    const request = new Request(`/change/ferme/${idFerme}`, {
                        method: "POST",
                        headers: {
                            'Accept': 'application/json',
                            'Content-Type': 'application/json'
                    ...
                    fetch(request).then(r => {
                        if (r.ok && r.status === 200) {
                            // alert("Fermes updated successfully")
...
function showNemberOfAvisFerme(idFerme,parent) {
    fetch(`/nombre/avis/ferme/${idFerme}`).then(r => {
        r.json().then(json => {
            const nombreAvis = json["nombre_avis"]
            createNombreAvisContainerFerme(parent, nombreAvis) 
        })
    })

}

function showNoteGlobaleFerme(idFerme) { 
    fetch(`/avis/ferme/global/${idFerme}`, {
        methode:"GET"
    }).then(r => {
        r.json().then(jsons => {
            let globalNote=0.00
            if (jsons) {
                ...
                createGlobalNoteFerme(globalNote)
            }
        })
    })
}
...
function createGlobalNoteFerme(globalNote) {
...
function createNombreAvisContainerFerme(parent,nombre) {
...
function mustBeInferior4Ferme(value,target, isThrowException) {
    regex=/[^0-9,\.]+/
...
function showAvisFerme(currentUserId,idFerme) {
    fetch(`/avis/ferme/global/${idFerme}`, {
        methode:"GET"
    }).then(r => {
        r.json().then(jsons => {
            if (jsons) {
                for (let json of jsons) {
                    createShowAvisFermeAreas(json,currentUserId)
                }
            }
        })
    })
...
function showModifAreaFerme(idFerme,currentUserId) {
    fetch(`/aviss/ferme/${idFerme}`, {
        methode:"GET"
    }).then(r => {
        r.json().then(jsons => {
            if (jsons) {
                ...
                for (let json of jsons) { 
                    ...
                    if (b) {
                        if (document.querySelector("#givs-avis-ferme-tom-js").style.display != "none") {
...
function createShowAvisFermeAreas(json,currentUserId) {
    console.log(json)
    const divEtAndModf=document.createElement("div")
    divEtAndModf.setAttribute("class", "fIQYlfMOP row")
