//scroll_to_bottom = document.querySelector('.content_message');
//scroll_to_bottom.scrollTop = scroll_to_bottom.scrollHeight;

//// ON THE PAGE CHAT BOX

///check btn send and input msg
if(document.querySelector(".btn_send_message_js_jheo_mobile") && document.querySelector(".input_message_js_jheo_mobile")){

    //// input message
    const message_input_mobile = document.querySelector(".input_message_js_jheo_mobile")

    ///btn send message
    const btn_send_message_mob = document.querySelector(".btn_send_message_js_jheo_mobile");

    ///input file
    const icon_input_file= document.querySelector(".outer_icon_message_mobile");
    ...
    icon_input_file.querySelector("input").addEventListener("change", (e) => {
        ...
        reader.addEventListener("load", () => {
            ...
            document.querySelector(".content_image_input_js_jheo_mobile").style.display= "flex"

            const parentImage = document.querySelector(".content_image_input_js_jheo_mobile")
            ...
            if(parentImage.querySelector("img")){
                ...
                document.querySelector(".content_image_input_js_jheo_mobile").appendChild(img)
            }
    ...
    message_input_mobile.addEventListener("keyup" , (e) => {

        ///the user key entre ... 
        if (e.code === "Enter" || e.code === "NumpadEnter") {

            ///check input content text
            if(document.querySelector(".input_message_js_jheo_mobile").value.length > 1 || document.querySelectorAll(".content_image_input_js_jheo_mobile img").length > 0 ){

                ///send message---------------------------------------------------
                sendMessage(document.querySelector(".input_message_js_jheo_mobile").value, image_list)

                if( document.querySelectorAll(".content_image_input_js_jheo_mobile img").length > 0 ){
                    image_list = []
                }
            }

            ///delete focus
            document.querySelector(".input_message_js_jheo_mobile").blur()

            ///reset input
            document.querySelector(".input_message_js_jheo_mobile").value= null

            ///delete content image above ...
            if(document.querySelectorAll(".content_image_input_mobile img")){

                const image_sended = document.querySelectorAll(".content_image_input_mobile img");
                image_sended.forEach(element => {
                    element.parentElement.removeChild(element)
                })
                document.querySelector(".content_image_input_mobile").style.display= "none"
            }
    ...
    btn_send_message_mob.addEventListener("click" , () => {

        console.log("gfdfdgfgfg")
        ///check input content text
        if(document.querySelector(".input_message_js_jheo_mobile").value.length > 1 || image_list.length>0 ){
...
function handleMessageResponse(date, message,image_list=null,image,status){
    ...
    if(document.querySelector(".content_input_message_js_jheo_mobile")){

        const block_message =document.querySelector(".content_input_message_js_jheo_mobile").parentElement;
        block_message.insertBefore(div_message,document.querySelector(".content_input_message_js_jheo_mobile"))
    }
