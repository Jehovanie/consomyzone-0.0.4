...
function plotDays(daysInMonth, firstDayPosition, monthNames, dateElement, calendarTitle, currentDate) {
	let count = 1;
	...
	for (let i = 0; i < daysInMonth; i++) {
		dateElement.innerHTML += `<button class="calendar-dates-day" data-cookie-action="show-dialog" data-date="${
			currentDate.$y
		}-${currentDate.$M + 1}-${count}" >${count++}</button>`;
	}
	...
	if (datas.length > 0) {
        ...
        `;
			...
			for (let j = 0; j < dataInter.length; j++) {
				...
				if (j === dataInter.length - 1) classTimeLine = "timeline__item end";
				...
				let haveFile =
					data.file_path === "/uploads/users/agenda/files/document/" ||
					data.file_path === "/uploads/users/agenda/files/img/"
						? false
						: true;
          ...
          `
				...
				let isShare =
					isEnd === ""
						? `<i class="fa-solid fa-share-nodes cta_share_agenda ${isAlreadyShare} share_agenda_${data.id}" id="cta_share_agenda_${k}" data-agenda-share="${k}" ></i>`
						: "";
				/// ---------------------------
		...
		if (modalModif.length > 0)
			modalModif.forEach((item) => {
				item.onclick = (e) => {
					...
					if (cookieDialog) cookieDialog.showModal();
					...
					fetch(request).then((r) => {
						createChargement(
							document.querySelector(".loader_agenda_modif" + k),
							"chargement_content chargement_agenda"
						);
						if (r.status === 200 && r.ok) {
							r.json().then((json) => {
					...
					liveSearch(
						"lieuEventNonPastilled" + k,
						"dialog-apropos-resto-agenda",
						`#choix-resto-type-past${k}`,
						`loader_agenda_modif${k}`,
						k
					);
				};
			});
		...
		if (modalShare.length > 0)
			modalShare.forEach((item) => {
				item.onclick = (e) => {
					...
					if (sharedDialog) sharedDialog.showModal();
				};
			});
		//---------------------------
...
function showDetailResto(a, k = 0) {
	...
	let cuisine_du_monde =
		parseInt(a.cuisineMonde) === 1 ? `<i class="fa-solid fa-hat-chef"></i> cuisine du monde` : "";
	...
	let btn =
		k === 0
			? `setChoice('${a.id}','${a.denominationF}','${adresse}')`
			: `setChoice('${a.id}','${a.denominationF}','${adresse}',${k})`;
...
function setChoice(a = 0, b, c, k = 0) {
	if (document.querySelector("#adresseEvent")) document.querySelector("#adresseEvent").value = c;
	if (document.querySelector("#adresseEvent" + k)) document.querySelector("#adresseEvent" + k).value = c;
	...
	if (document.querySelector("#lieuEventNonPastilled")) document.querySelector("#lieuEventNonPastilled").value = b;
...
function liveSearch(
	idInputRestaurant = "lieuEventNonPastilled",
		...
		.then((response) => {
			...
			if (response.hasOwnProperty("status")) {
				if (response.status === "shareSuccess") {
					...
					if (tribuTChecked === null) {
						///not select tribu T
						const data_rank_modal = document
							.querySelector(".fa_solid_open_dialog_for_share_js_jheo")
							.getAttribute("data-rank");
						createHtmlViewListTribuT(
							data_rank_modal,
							response.all_tribugT,
							agendaID,
							shareFor,
							agendaToShare
						);
					...
					const data_rank_modal = document
						.querySelector(".fa_solid_open_dialog_for_share_js_jheo")
						.getAttribute("data-rank");
					createHtmlViewListPartisan(
						data_rank_modal,
						response.all_users_tribu,
						agendaID,
						shareFor,
						agendaToShare
					);
...
function createHtmlViewListTribuT(dataRankForShare, tabListTribuT, agendaID, shareFor, agendaToShare) {
	...
	///injection HTMl
	document.querySelector(`.content_dialog_for_share_agenda_share_${dataRankForShare}_js_jheo`).innerHTML =
		html_list_tribuT;
...
function bindEventForShareInTribuT(tabListTribuT, dataRankForShare, agendaID, shareFor, agendaToShare) {
	...
	if (document.querySelector(`.btn_send_share_agenda_tribuT_${dataRankForShare}_js_jheo`)) {
		const cta_send_share_agenda_tribuT = document.querySelector(
			`.btn_send_share_agenda_tribuT_${dataRankForShare}_js_jheo`
		);
...
function bindEventForShareForSamePartisan(usersList, dataRankForShare, agendaID, shareFor, agendaToShare) {
	if (document.querySelector(`.btn_send_share_agenda_list_partisan_${dataRankForShare}_js_jheo`)) {
		const cta_send_share_agenda = document.querySelector(
			`.btn_send_share_agenda_list_partisan_${dataRankForShare}_js_jheo`
		);
