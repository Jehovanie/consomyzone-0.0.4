const user_id = parseInt(
  document.querySelector(".main_user_id")?.getAttribute("id")
);
const fullname = document
  .querySelector(".main_user_id")
  ?.getAttribute("data-toggle-full-name");
...
if (document.querySelector(".content_publication_js_jheo")) {
  ...
  evtSource.onmessage = function (event) {
    ...
    all_publication.forEach((publication) => {
      ...
      const nbr_reaction =
        publication.reactions.length > 0 ? publication.reactions.length : "";
      const nbr_comment =
        publication.comments.length > 0 ? publication.comments.length : "";

      if (
        document.querySelector("#pubication_js_" + publication_id + "_jheo")
      ) {
        document.querySelector(
          "#nbr_reaction_pub_" + publication_id
        ).innerText = nbr_reaction;
        document.querySelector("#nbr_comment_pub_" + publication_id).innerText =
          nbr_comment;
      }
    });

    ///filter publications not show.
    const last_pub_id_show = document.querySelector(".publication_js_jheo")
      ? document
          .querySelector(".publication_js_jheo")
          .getAttribute("data-toggle-pub-id")
      : 0;
    const new_publication = all_publication.filter(
      (pub) => parseInt(pub.id) > parseInt(last_pub_id_show)
    );

    if (
      new_publication.length > 0 &&
      document.querySelector(".content_publication_js_jheo")
    ) {
      if (
        document
          .querySelector(".btn_actualise_js_jheo")
          .classList.contains("hidden_actualise")
      ) {
        document
          .querySelector(".btn_actualise_js_jheo")
          .classList.remove("hidden_actualise");
      }

      if (
        !document
          .querySelector(".btn_actualise_js_jheo")
          .classList.contains("btn_actulise_annimation")
      ) {
        document
          .querySelector(".btn_actualise_js_jheo")
          .classList.add("btn_actulise_annimation");
      }

      setTimeout(() => {
        document
          .querySelector(".btn_actualise_js_jheo")
          .classList.add("hidden_actualise");
        document
          .querySelector(".btn_actualise_js_jheo")
          .classList.remove("btn_actulise_annimation");
      }, 5000);
    }
  };
}
...
if (document.querySelector(".content_chargement_publication_js_jheo")) {
  window.addEventListener("scroll", function (e) {
    ...
    const content_description = document.querySelector(
      ".need_position_fixed_js_jheo"
    );
    const scroll_goal =
      content_description.getBoundingClientRect().bottom +
      content_description.getBoundingClientRect().height;

    if (parseInt(scroll_goal) - parseInt(scroll_Y) < 0) {
      if (
        !document
          .querySelector(".need_position_fixed_js_jheo")
          .classList.contains("position_sticky")
      ) {
        document
          .querySelector(".need_position_fixed_js_jheo")
          .classList.add("position_sticky");
      }
    } else {
      if (
        document
        ...
        document
          .querySelector(".need_position_fixed_js_jheo")
          .classList.remove("position_sticky");
      }
    }

    if (parseInt(scroll_Y) === parseInt(scrollable)) {
      const loading_content = document.querySelector(
        ".content_chargement_publication_js_jheo"
      );
      const last_pub_id = loading_content.getAttribute("data-toggle-pub-id");

      const content_pub = document.querySelector(
        ".content_publication_js_jheo"
      );

      fetch(`/tribuG/publications/${parseInt(last_pub_id)}`)
        .then((response) => response.text())
        .then((response) => {
          var responseHTML = new DOMParser().parseFromString(
            response,
            "text/html"
          );
          ...
          if (responseHTML.querySelector(".publication_js_jheo")) {
            document
              .querySelector(".content_chargement_publication_js_jheo")
              .remove();
...
if (document.querySelector(".send_new_pub_js_jheo")) {
  document
    .querySelector(".send_new_pub_js_jheo")
...
function handleCommentPublished(
  ...
  if (bouton) {
    audioSrc =
      bouton.querySelector("#audio_" + publication_id) != null
        ? bouton.querySelector("#audio_" + publication_id).src
        : "";
...
function fetchNotification(publication_id) {
  const content_comment = document
    .querySelector(`.content_comment_${publication_id}_js_jheo`)
    .querySelector(".content_notification_tributg");
  ...
  document
    .querySelector(`.content_comment_${publication_id}_js_jheo`)
    .querySelector(".content_notification_tributg").innerHTML = `
  ...
  fetch("/tributG/publications/comments/fetchall", options_request)
    ...
    .then((results) => {
      if (
        document
          .querySelector(`.content_comment_${publication_id}_js_jheo`)
          .querySelector(".content_loading_js_jheo")
      ) {
        document
          .querySelector(`.content_comment_${publication_id}_js_jheo`)
          .querySelector(".content_loading_js_jheo")
...
function createCardComment(
  ...
  if (
    content_comment &&
    content_comment.querySelectorAll(".card_comment_js").length > 0
  ) {
...
function changeComment(comment_id, publication_id) {
  const card_comment = document.querySelector(
    `.card_comment_${comment_id}_js_jheo`
  );
  document.querySelector(`.text_input_${publication_id}_js_jheo`).value =
    card_comment.querySelector(".text_js_jheo").innerText;

  document
    .querySelector(`.cta_send_notification_${publication_id}_js_jheo`)
    .setAttribute(
      "onclick",
      `handleChangeComment("${publication_id}", "${comment_id}")`
    );
...
function show_modal_remove_com(pub_id, comm_id) {
  document
    .querySelector(".cta_confirme_del_com_js_jheo")
    .setAttribute("onclick", `removeComment('${pub_id}', '${comm_id}')`);
}
...
function handleAndSentNotification(publication_id, publication_user_id) {
  ...
  if (document.querySelector(".form_comment_" + publication_id)) {
    const form_content = document.querySelector(
      ".form_comment_" + publication_id
    );

    if (
      form_content.querySelector("input").value != null &&
      form_content.querySelector("input").value.length > 1
    ) {
      const user_id = parseInt(
        document.querySelector(".main_user_id").getAttribute("id")
      );
...
function handleChangeComment(publication_id, comment_id) {
  ...
  if (
    ...
    const post_publication = document.querySelector(
      `#pubication_js_${publication_id}_jheo`
    );
    const pub_user_id = parseInt(
      post_publication.getAttribute("data-toggle-pub-user-id")
    );
    ...
    document
      ...
      .setAttribute(
        "onclick",
        `handleAndSentNotification('${publication_id}', '${pub_user_id}')`
      );
...
function handleEditComment(publication_id, comment_id, text_comment) {
  ...
  fetch(
    `/tributG/publications/${publication_id}/comment/${comment_id}/change`,
    request_headers
  )
    .then((response) => response.json())
...
// }
function updatePublication() {
  let img_data = document.querySelector("#updtMdlPub").dataset.url;
  let data = {
    pub_id: document.querySelector("#hiddenElementUpdate").value,
    confidentiality: document.querySelector(
      "#publication_update_confidentiality"
    ).value,
    message: document.querySelector("#publication_update_legend").value,
    table: document.querySelector("#hiddenElementUpdate").dataset.name,
    img_data: img_data,
  };
  ...
  fetch(URL, options)
    ...
    .then((response) => {
      $("#modal_publication_modif").modal("hide");
      swal({
        title: "Bravo !",
        text: "Votre publication a été bien à jour !",
        icon: "success",
        button: "Fermer",
      }).then((value) => {
...
function addAudio(bouton, id) {
  ...
  let dateString =
    dateForName.getDate() + "_" + month + "_" + dateForName.getFullYear();
          ...
          mediaRecorder.onstop = (e) => {
            ...
            deleteButton.onclick = (e) => {
              ...
              evtTgt.parentNode.parentNode.removeChild(
                evtTgt.parentNode.previousElementSibling
              );
...
function setHiddenValue(table, update = "", pub_id) {
  let parentOfElement = document.querySelector("#" + table + "_" + pub_id);
