...
function endChat() {
	...
	if (
		document.querySelector("div.user-chat-display").getAttribute("data-user-id") == "0" &&
		...
		evtSource.onmessage = function (event) {
			...
			all_messages.forEach((message) => {
				...
				if (
					...
					document.querySelector("div.user-chat-display").getAttribute("data-user-id") == user_id
				) {
					let img_doc = "";
...
let user_name = document.querySelector("#my_full_name")
	...
	? document.querySelector("span.jc.un.mn.zn.gs.use-in-agd-nanta_js_css").textContent.trim()
	: "";
...
function joinMeet(...args) {
	try {
		...
		if (document.querySelector("#visioMessageElie").classList.contains("d-none")) {
		...
		apiJitsi.on("readyToClose", () => {
			...
			if (currentUrl.includes("user/message")) {
				...
				if (args[2]) {
					if (args[3]) {
						node = args[2];
						message_id = node.parentElement.parentElement.parentElement.parentElement
							.getAttribute("id")
							.replaceAll(/[^0-9]/g, "");
						node.parentElement.parentElement.parentElement.parentElement.innerHTML = content;
					} else {
...
function createVisioGroupFromMessage() {
	...
	document.querySelector("#confirmVisioGroupForMessage").addEventListener("click", function () {
		// let roomGroup = "Meet" + generateUID() + document.querySelector("#amis_list").getAttribute("data-my-id")
		let roomGroup =
			"Meet" + generateUID() + document.querySelector(".my-profile-id-elie").getAttribute("data-my-id");
...
function createListeFans(
	...
	if (ulClass != "") ul.setAttribute("class", ulClass);

	const modalContainer = document.querySelector("#" + id);
	if (!addNewElement) modalContainer.innerHTML = "";
  ...
  `;
	...
	if (useSearchBar) {
		if (document.querySelector(".back_to_futur")) document.querySelector(".back_to_futur").remove();
...
function updateList(datas) {
	...
	createListeFans(
		"modal_Partage_Message_Faniry_js > .result_search",
		finalResult,
...
async function createFanListToShareMessage(event) {
	...
	createListeFans(
		"modal_Partage_Message_Faniry_js > .contacts",
		fans,
		"fan_who_selected_prtg_msgfaniry_js",
		true,
		"Contacts",
				...
				if (e.target.checked) {
					...
					if (inputs.length > 0) {
						...
						Array.from(inputs).forEach((input) => {
							input.checked = true;
							if (input.checked) isSomeChecked.push(true);
						});
					...
					if (inputs.length > 0) {
						...
						Array.from(inputs).forEach((input) => {
							input.checked = false;
							if (!input.checked) isSomeChecked.push(false);
						});
						if (isSomeChecked.indexOf(true) < 0) {
							document.querySelector("#patager_message_faniry_js").disabled = true;
						} else {
...
async function createVisoGroup() {
	...
	fans.forEach((item) => {
		item.forEach((jtem) => {
			if (jtem.hasOwnProperty("amis")) {
				data_temp = [...data_temp, ...jtem.amis];
			} else if (jtem.hasOwnProperty("my_id")) {
				data_temp = [...data_temp, ...jtem];
			}
		});
	});
	...
	data_temp.forEach((item) => {
		result.push({
			id_fan: parseInt(item.id),
			photo_fan: item.hasOwnProperty("photo")
				? item.hasOwnProperty("photo")
				: item.hasOwnProperty("image_profil"),
	...
	createListeFans("visioGroupContentForMessage", result);
		...
		if (e.key === "Enter" || e.keyCode === 13) {
			if (document.querySelector("div.user-chat-display").getAttribute("data-user-id") == 0) {
				...
				sendChat(
					e.target.value,
					image_list,
					document.querySelector("div.user-chat-display").getAttribute("data-user-id")
				);
			}
			...
			if (document.querySelector("#chat_container").getAttribute("data-type") != "visio") {
				...
				if (user_name != "Assistant Virtuel") {
					...
					document
						...
						.setAttribute("data-user-id", amis.getAttribute("data-toggle-user-id"));
				...
				let roomRandom =
					"Meet" + generateUID() + document.querySelector("#amis_list").getAttribute("data-my-id");
...
if (document.querySelector("#canceled_message_shared_faniry_js")) {
	document.querySelector("#canceled_message_shared_faniry_js").addEventListener("click", () => {
		if (document.querySelectorAll(".selected_all_fans_js").length > 0) {
			Array.from(document.querySelectorAll(".selected_all_fans_js")).forEach((item) => {
...
if (document.querySelector("#close_message_shared_faniry_js")) {
	document.querySelector("#close_message_shared_faniry_js").addEventListener("click", () => {
		if (document.querySelectorAll(".selected_all_fans_js").length > 0) {
			Array.from(document.querySelectorAll(".selected_all_fans_js")).forEach((item) => {
				if (item.checked) item.checked = false;
			});
...
if (document.querySelector("#canceled_visio_shared_faniry_js")) {
	document.querySelector("#canceled_visio_shared_faniry_js").addEventListener("click", () => {
		if (document.querySelectorAll(".selected_all_fans_js").length > 0) {
			Array.from(document.querySelectorAll(".selected_all_fans_js")).forEach((item) => {
				if (item.checked) item.checked = false;
			});
...
if (document.querySelector("#close_visio_shared_faniry_js")) {
	document.querySelector("#close_visio_shared_faniry_js").addEventListener("click", () => {
		if (document.querySelectorAll(".selected_all_fans_js").length > 0) {
			Array.from(document.querySelectorAll(".selected_all_fans_js")).forEach((item) => {
				if (item.checked) item.checked = false;
			});
...
if (document.querySelector(".information_user_conected_jheo_js")) {
	...
	if (document.querySelector("#openMessage")) {
		...
		event_source_show_message.onmessage = function (event) {
			...
			const message_not_show = new_message.filter((item) => {
