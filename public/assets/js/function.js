...
function addRestaurantToMap(nom_dep, code_dep) {
  ...
  document.querySelectorAll(".element_js_jheo").forEach((item) => {
    ...
    geos.push(
      franceGeo.features.find((element) => element.properties.code == dep)
    );
  });
  ...
  if (id_resto_url) {
    fetch(API_URL)
      ...
      .then((response1) => {
        ...
        response1.forEach((item) => {
          ...
          const adress =
            "<br><span class='fw-bolder'> Adresse:</span> <br>" +
            adresseRestaurant;
          ...
          if (item.id != id_resto_url) {
            marker = L.marker(
              ...
              { icon: setIconn("assets/icon/icon-resto-bleu.png") }
            );
          } else {
            marker = L.marker(
              L.latLng(parseFloat(item.poiY), parseFloat(item.poiX)),
              { icon: setIconn("assets/icon/icon-restoR.png") }
            );
          ...
          marker.on("click", (e) => {
            ...
            if (screemMax.matches) {
              ...
              fetch(
                `/restaurant/departement/${departementName}/${item.dep}/details/${item.id}`,
                myHeaders
              )
                ...
                .then((r) => {
                  ...
                  document
                    .querySelector("#close-detail-tous-resto")
                    .addEventListener("click", () => {
                      document.querySelector("#content-details").style.display =
                        "none";
                    });
                  document
                    .querySelector("#content-details")
        ...
        if (nom_dep && code_dep) {
          ...
          parsedResult.forEach((new_element) => {
            ...
            s_p.innerHTML =
              "<span class='id_departement'>" +
              new_element.nomFerme +
              " </span>" +
              new_element.adresseFerme;
    ...
    fetch(API_URL)
      ...
      .then((response1) => {
          ...
          iconCreateFunction: function (cluster) {
            if (marker_last_selected) {
              ...
              for (let g of cluster.getAllChildMarkers()) {
                if (
                  parseInt(marker_last_selected.options.id) ===
        ...
        response1.forEach((item) => {
          ...
          marker.on("click", (e) => {
            ...
            const icon_R = L.Icon.extend({
              options: {
                iconUrl: IS_DEV_MODE
                  ? url.origin + "/assets/icon/NewIcons/icon-resto-new-Rr.png"
                  : url.origin +
                    "/public/assets/icon/NewIcons/icon-resto-new-Rr.png",
                iconSize: [32, 50],
              },
            });
            ...
            if (marker_last_selected && marker_last_selected != marker) {
              const icon_B = L.Icon.extend({
                options: {
                  iconUrl: IS_DEV_MODE
                    ? url.origin + "/assets/icon/NewIcons/icon-resto-new-B.png"
                    : url.origin +
                      "/public/assets/icon/NewIcons/icon-resto-new-B.png",
                  iconSize: [32, 50],
                },
              });
        ...
        cta_setCurrentPosition.className =
          "cta_setCurrentPosition cta_setCurrentPosition_jheo_js";
    ...
    res.json().then((responses) => {
      ...
      responses.restaurants.forEach((response) => {
        ...
        listSpecMobile.innerHTML += `
                    <li class="nav-item icon-tabac me-3 content_avie_details_tomm_js " data-toggle-id-resto="${
                      response.id
                    }">
          ...
          response.depName
                                ...
                                <div class="d-flex justify-content-center align-items-center flex-gap-2 content_btn_avis">
                                    <span>
                                        <a id="see-tom-js${
                                          response.id
                                        }" class="text-black text-point-9 btn btn-warning" data-bs-toggle="modal" data-bs-target="#staticBackdrop${
          response.id
        }" onclick="showListAvieMobile(${response.id}, ${id_user})">
                    ...
                    <div class="modal fade list-avis-ferme-global-mobile" id="staticBackdrop${
                      response.id
                        ...
                        <div class="modal-dialog  modal-dialog-scrollable">
                            <div class="modal-content">
                                <div class="modal-header bg-light">
                                    ...
                                    <button type="button" class="btn-close" onclick="closeModalAvieDetail(${
                                      response.id
                                    })"></button>
                                </div>
                                <div class="list-avis-ferme">
                                    <div class="modal-body container-avis all_avis_${
                                      response.id
                                    }_jheo_js">
                                    </div>
                                </div>
                    ...
                    <div class="modal fade modal-avie-resto-mobile-tomm-js" id="modalAvisRestaurant${
                      response.id
                    }" tabindex="-1" aria-labelledby="modalAvisRestaurantLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="modalAvisRestaurantLabel">Votre Avis</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" data-bs-toggle="modal" data-bs-target="#staticBackdrop${
                                      response.id
                                    }" onclick="showListAvieMobile(${
          response.id
        }, ${id_user})"></button>
                                </div>
                                <div class="modal-body">
                                    <form>
                                        ...
                                        <textarea class="form-control note_number_${
                                          response.id
                                        }_jheo_js text-note-mobile-tomm-js" id="text-note-mobile-${
          response.id
        }"></textarea>
                                        <label for="message-text" class="col-form-label">Commentaire:</label>
                                        <textarea class="form-control note_avis_${
                                          response.id
                                        }_jheo_js" id="message-text-mobile-${
          response.id
        }"></textarea>
                                    </form>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-warning send_avis_${
                                      response.id
                                    }_jheo_js"  data-bs-dismiss="modal" data-bs-toggle="modal"   data-bs-target="#staticBackdrop${
          response.id
            ...
            if (items.querySelector(".flash-msg-ERREUR")) {
              items
                ...
                .parentNode.removeChild(
                  items.querySelector(".flash-msg-ERREUR")
                );
            }
