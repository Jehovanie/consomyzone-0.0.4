	...
	fetch(`/ferme-mobile/departement/${nom_dep}/${id_dep}`)
		...
		.then((r) => {
			...
			document.querySelector("#close-ferme-specific").addEventListener("click", () => {
				...
				document.querySelector("#open-navleft-mobile-tomm-js-specific").style.transition =
				...
				.then((r) => {
					if (document.querySelector(".list-depart-resto-mobile-tomm-js")) {
						...
						document.querySelector("#close-resto").addEventListener("click", () => {
							document.querySelector(".list-depart-resto-mobile-tomm-js").style =
								"transform: translateX(-100vw);display: none;";
...
function setPhotoTribu(btn_photo) {
	if (btn_photo.tagName != "IMG") {
		if (document.querySelector("#img_modal")) {
			...
			document.querySelector("#img_modal_actu").src = btn_photo.querySelector("img").src;
		}
	} else {
		if (document.querySelector("#img_modal")) {
...
function calculateDurationOfComment(dateOfComment) {
	...
	if (dateOfComment.split(" ")[0] != dateStr) {
		...
		let lapsTime = Math.abs(
			parseInt(dateOfComment.split(" ")[1].split(":")[0], 10) - parseInt(hour.split(":")[0], 10)
		);
		if (lapsTime == 0) {
...
function removeUpload() {
	if (document.querySelector("#updtMdlPub")) document.querySelector("#updtMdlPub").dataset.url = "";
	...
	$(".drag_text_jheo_js").html(`
        <img src="/public/assets/image/uplaodIcon.png" alt="background upload file">
		...
		fetch(request)
			...
			.then((response) => {
				...
				publicationItems.querySelector(".nbr_comment_jheo_js").innerText =
					new_nbr_comment > 1 ? `${new_nbr_comment} commentaires` : `${new_nbr_comment} commentaire`;
				...
				.then((r) => {
					if (document.querySelector("#list-depart-golf-mobile-tomm-js")) {
						...
						document.querySelector("#close-golf-dep").addEventListener("click", () => {
							document.querySelector("#list-depart-golf-mobile-tomm-js").style =
								"transform: translateX(-100vw);display: none;";
...
function addSpecificgolfMobile(nom_dep, id_dep) {
	if (document.querySelector("#open-navleft-golf-mobile-specific-tomm-js")) {
		...
		document.querySelector("#open-navleft-golf-mobile-specific-tomm-js").style.transition =
			"opacity 0.5s ease-in-out";
			...
			.then((r) => {
				if (document.querySelector("#list-depart-golf-specific-mobile-tomm-js")) {
					...
					document.querySelector("#close-golf-specific-tomm-js").addEventListener("click", () => {
						document.querySelector("#list-depart-golf-specific-mobile-tomm-js").style =
							"transform: translateX(-100vw);display: none;";
...
function addListDepartStation() {
	...
	if (document.querySelector("#open-navleft-station-mobile-tomm-js")) {
		document.querySelector("#open-navleft-station-mobile-tomm-js").addEventListener("click", () => {
			...
			document.querySelector("#open-navleft-station-mobile-tomm-js").style.transition =
				"opacity 0.5s ease-in-out";
				...
				.then((r) => {
					if (document.querySelector("#list-depart-station-mobile-tomm-js")) {
						...
						document.querySelector("#close-station-dep").addEventListener("click", () => {
							document.querySelector("#list-depart-station-mobile-tomm-js").style =
								"transform: translateX(-100vw);display: none;";
...
function addSpecificStationMobile(nom_dep, id_dep) {
	if (document.querySelector("#open-navleft-station-mobile-specific-tomm-js")) {
		...
		document.querySelector("#open-navleft-station-mobile-specific-tomm-js").style.transition =
			"opacity 0.5s ease-in-out";
			...
			.then((r) => {
				if (document.querySelector("#list-depart-station-specific-mobile-tomm-js")) {
					...
					document.querySelector("#close-station-specific-tomm-js").addEventListener("click", () => {
						document.querySelector("#list-depart-station-specific-mobile-tomm-js").style =
							"transform: translateX(-100vw);display: none;";
				...
				.then((r) => {
					if (document.querySelector("#list-depart-tabac-mobile-tomm-js")) {
						...
						document.querySelector("#close-tabac-dep").addEventListener("click", () => {
							document.querySelector("#list-depart-tabac-mobile-tomm-js").style =
								"transform: translateX(-100vw);display: none;";
		...
		.then((r) => {
			if (document.querySelector("#list-depart-tabac-mobile-spec-tomm-js")) {
				...
				document.querySelector("#close-tabac-spec-dep").addEventListener("click", () => {
					document.querySelector("#list-depart-tabac-mobile-spec-tomm-js").style =
						"transform: translateX(-100vw);display: none;";
			...
			response.json().then((response) => {
				...
				if (data.length > 0) {
					data.forEach((item) => {
						...
						let logo = item.logo_path ? item.logo_path : "/uploads/tribu_t/photo/avatar_tribu.jpg";
		...
		.then((response) => {
			...
			if (isOk) {
				document
					...
					.setAttribute(
						"onclick",
						"pastilleForTribuG(this, false," + id + ",'" + e.target.dataset.name + "')"
					);
				...
				document
					...
					.setAttribute(
						"onclick",
						"pastilleForTribuG(this, true, " + id + ",'" + e.target.dataset.name + "')"
					);
...
if (document.querySelector(".scroll-mobile-tomm-js")) {
	...
	contentSpecMobile.addEventListener("scroll", (event) => {
		...
		if (
			Math.ceil(contentSpecMobile.scrollWidth - contentSpecMobile.clientWidth) ===
	...
	fetch(request)
		...
		.then((response) => {
			...
			let restaurantAvisNote =
				restaurants.avis.note !== 0 ? restaurants.avis.note - (restaurants.avis.note - 1) : 0;
			// Calculez le taux et les pourcentages jaunes et noirs
			let rate = restaurantAvisNote;
			let rateYellow = rate * 100;
			let rateBlack = 100 - rateYellow;
...
function getDataSpecStationMobile(nom_dep, id_dep) {
	...
	fetch(request)
		.then((res) => res.json())
		.then((responses) => {
			if (document.querySelector(".loading-tomm-js")) document.querySelector(".loading-tomm-js").innerHTML = "";
			...
			responses.golf.forEach((response) => {
				...
				if (document.querySelector("#is-connected-tomm-js")) {
					...
					if (response.user_status["a_faire"] == null && response.user_status["fait"] == null) {
						valueContaintGolf = ` 
                                <label for="selectActionGolf" class="form-label">Vous voulez marquer que ce golf comme : </label>
                                <select class="form-select select_action_golf select_action_golf_nanta_js" id="selectActionGolf" name="sellist_action" data-id="${response.id}" onchange="executeActionForPastGolf(event, ${response.id})">
                                    <option value="0">Aucun</option>
                                    <option value="1">A faire</option>
                                    <option value="2">Fait</option>
                                    <option value="3">A refaire</option>
						...
						valueContaintGolfDetail = ` 
                                <label for="selectActionGolf" class="form-label">Vous voulez marquer que ce golf comme :</label>
                                    <select class="form-select-detail select_action_golf select_action_golf_nanta_js" id="selectActionGolf" name="sellist_action" data-id="${response.id}" onchange="executeActionForPastGolf(event, ${response.id})">
                                        <option value="0">Aucun</option>
                                        <option value="1">A faire</option>
                                        <option value="2">Fait</option>
                                      <option value="3">A refaire</option>
						...
						valueContaintGolf = `Voulez-vous annuler votre choix ? <span class="badge bg-danger btn_golf_did  btn_golf_did_jheo_js" onclick="cancelGolfFinished(event, '${response.id}')">Oui</span>`;
						valueContaintGolfDetail = `Voulez-vous annuler votre choix ? <span class="badge bg-danger btn_golf_did  btn_golf_did_jheo_js" onclick="cancelGolfFinished(event, '${response.id}')">Oui</span>`;
...
function showModalPicture() {
	...
	navigator.mediaDevices
		...
		.catch((error) => {
			swal("Attention !", "Impossible d'accéder à votre caméra !", "warning").then((res) => {
				$("#mediaModalTribu").modal("hide");
				$("#modal_publication_modif").modal("show");
			});
		});
 ...
 */
function showModalPictureTribu(type, isUpdate = false) {
	document.querySelector("#containerCamera").innerHTML = "";
	// <video id="player" autoplay></video>
	let video = document.createElement("video");
	video.setAttribute("id", "player");
	...
	if (isUpdate && type == "pub") {
		captureButton.setAttribute("onclick", "");
		captureButton.setAttribute("onclick", "takePictureTribu('pub',true)");
		document.querySelector("#mediaModalTribu .modal-header > button").dataset.bsTarget = "#modal_publication_modif";
	} else if (!isUpdate && type == "pub") {
		captureButton.setAttribute("onclick", "");
		captureButton.setAttribute("onclick", "takePictureTribu('pub')");

		if (document.querySelector("#modal_publication_tributG"))
			document.querySelector("#mediaModalTribu .modal-header > button").dataset.bsTarget =
				"#modal_publication_tributG";
		if (document.querySelector("#newPublication"))
			document.querySelector("#mediaModalTribu .modal-header > button").dataset.bsTarget = "#newPublication";
		if (document.querySelector("#modal_publication"))
			document.querySelector("#mediaModalTribu .modal-header > button").dataset.bsTarget = "#modal_publication";
	}
		...
		.catch((error) => {
			swal("Attention !", "Impossible d'accéder à votre caméra !", "warning").then((res) => {
				...
				if (isUpdate && type == "pub") {
					...
					if (document.querySelector("#newPublication")) $("#newPublication").modal("show");
					if (document.querySelector("#modal_publication")) $("#modal_publication").modal("show");
				}
			});
		});
 ...
 */
function takePictureTribu(type, isModif = false) {
	...
	if (isModif) {
		document.querySelector("#modal_publication_modif img.image_upload_image_jheo_js").src = data;
		document.querySelector("#updtMdlPub").dataset.url = data;
		$("#modal_publication_modif").modal("show");
		document
			.querySelector("#modal_publication_modif #imageUploadTribu > div.image-upload-wrap")
			.classList.add("d-none");
		document.querySelector("#modal_publication_modif #imageUploadTribu > div.image-upload-content").style.display =
			"block";
	} else {
		let preview;
	...
	fetch(request)
		...
		.then((message) => {
			new swal("Succès !", "Golf pastillé avec succès", "success").then((value) => {
				...
				/// this is not use in the cart map.
				showGolf(tbl);
				...
				if (document.querySelector(".content_tribuT_pastille_jheo_js")) {
					...
					img.src = element.dataset.velona;
					img.dataset.name = element.dataset.name;
					img.dataset.type = "tribuT";
					img.setAttribute("alt", element.dataset.name);
					...
					div.setAttribute("data-bs-toggle", "tooltip");
					div.setAttribute("data-bs-placement", "top");
					div.setAttribute("title", "Tribu T " + element.dataset.name);
					div.setAttribute("data-name", element.dataset.name);
					...
					if (
						document.querySelector(".row_content_tribu_pastille_jheo_js") &&
						document.querySelector(".row_content_tribu_pastille_jheo_js").classList.contains("d-none")
					) {
						document.querySelector(".row_content_tribu_pastille_jheo_js").classList.remove("d-none");
					}
		...
		.then((message) => {
			new swal("Succès !", "Golf dépastillé avec succès", "success").then((value) => {
				...
				if (document.querySelector(".content_tribuT_pastille_jheo_js")) {
					...
					if (all_tribuT_pastille.length < 4) {
						if (
							!content_tribuT_pastille.querySelector(".iconePlus_nanta_js").classList.contains("d-none")
						) {
							content_tribuT_pastille.querySelector(".iconePlus_nanta_js").classList.add("d-none");
						}
					}
				}

				if (document.querySelectorAll(".content_logo_tribu_pastille_jheo_js").length === 0) {
					if (
						document.querySelector(".row_content_tribu_pastille_jheo_js") &&
						!document.querySelector(".row_content_tribu_pastille_jheo_js").classList.contains("d-none")
					) {
						document.querySelector(".row_content_tribu_pastille_jheo_js").classList.add("d-none");
					}
				}
...
function showPastillGolfTribuT(id_golf, name_golf, adress_golf) {
	fetch(`/golf/pastilled/checking/${id_golf}`)
		...
		.then((datas) => {
			...
			if (datas.length > 0) {
				datas.forEach((data) => {
					let logo = data["logo_path"] != "" ? data["logo_path"] : "/uploads/tribu_t/photo/avatar_tribu.jpg";
					logo = IS_DEV_MODE ? logo : "/public" + logo;

					const logoPath = `<img class="logo_path_pastille" src="${logo}" />`;
					...
					const btnPastille = `
						<button type="button" 
							...
							data-velona="${logo}"
							data-tbname=${data.table_name} 
							onclick="pastilleGolf(this, '${data.table_name}')">
							Pastillez
						</button>
					`;

					const btn = data.isPastilled == true ? btnDePastille : btnPastille;
			...
			fetch(`/user/tribu_g/isPastilled/${tribu_g_name + "_golf"}/${id_golf}`)
				...
				.then((response) => {
					...
					const logo = profil_tribuG.logo_path
						? "/uploads/tribus/photos/" + profil_tribuG.logo_path
						: "/uploads/tribu_t/photo/avatar_tribu.jpg";

					const logoTribuG = IS_DEV_MODE ? logo : "/public" + logo;
              ...
              <div class="m-2 content_list_resto_js_g d-none">
                  <table style="width: 100%;">
										...
										<td class="col-tribuT">${profil_tribuG.name_tribu_t_muable}</td>
										<td class="col-action">
											<button id="btn-pastille-elie-tbg" 
												class="btn btn-${classe} btn-sm" 
												data-tbname ="${tribu_g_name}"
												data-name ="${name_golf.replaceAll("'", "\\'")}"
												data-id= "${id_golf}"
												data-velona= "${logoTribuG}"
					...
					if (action === "finished") {
						new swal("Bravo !", "Vous avez marqué ce golf comme fait !", "success").then((value) => {
							...
							if (document.querySelector(`.item_carrousel_${goldID}_jheo_js`)) {
								const li_on_carrousel = document.querySelector(`.item_carrousel_${goldID}_jheo_js`);
								const other_selectElement = li_on_carrousel.querySelector(
									".content_btn_golf_did_jheo_js"
								);

								other_selectElement.innerHTML = `
                                        Voulez-vous annuler votre choix ? <span class="badge bg-danger btn_golf_did btn_golf_did_jheo_js" onclick="cancelGolfFinished(event,${goldID})">Oui</span>
                                    `;
							}
						...
						new swal("Bravo !", "Vous avez choisi de ne rien faire avec ce golf.", "success").then(
							(value) => {
								if (document.querySelector(".content_btn_golf_did_jheo_js")) {
									selectElement.innerHTML = `
                                        Voulez-vous annuler votre choix ? <span class="badge bg-danger btn_golf_did btn_golf_did_jheo_js" onclick="cancelGolfFinished(event,${goldID})">Oui</span>
						...
						new swal("Info !", "Vous venez d'annuler votre choix !", "success").then((value) => {
							if (document.querySelector(".content_btn_golf_did_jheo_js")) {
								selectElement.innerHTML = `
                                    <label for="selectActionGolf" class="form-label">Vous voulez marquer que ce golf comme:</label>
				...
				if (res.status == "ok") {
					swal({
						...
						if (document.querySelector(".mainContainerLogoTribu")) {
							...
							div.setAttribute("class", "img_nantenaina content_logo_tribu_pastille");
							...
							if (document.querySelector(".content_tribuG_pastille_jheo_js")) {
								const content_tribuG_pastille = document.querySelector(
									".content_tribuG_pastille_jheo_js"
								);
			...
			.then((res) => {
				if (res.status == "ok") {
					swal({
						...
						if (document.querySelector(".content_tribuG_pastille_jheo_js")) {
							const content_tribuG_pastille = document.querySelector(".content_tribuG_pastille_jheo_js");
							const info_tribuG = res.profil_tribuG;

							let img = document.createElement("img");
							img.id = info_tribuG["table_name"] + "_image";
							img.setAttribute(
								"class",
								`logo_tribu_pastille logo_path_${info_tribuG["table_name"]}_tomm_js logo_path_pastille_details-tomm-js`
							);

							let logo_path =
								info_tribuG["logo_path"] === null
									? "/uploads/tribu_t/photo/avatar_tribu.jpg"
									: "/uploads/tribus/photos/" + info_tribuG["logo_path"];

							logo_path = IS_DEV_MODE ? logo_path : "/public" + logo_path;

							img.src = logo_path;

							img.setAttribute("data-name", info_tribuG["name_tribu_t_muable"]);
							img.setAttribute("alt", "Icon Tribu G");

							let div = document.createElement("div");
							div.setAttribute("id", info_tribuG["table_name"]);
							div.setAttribute("class", "img_nantenaina content_logo_tribu_pastille");
							div.setAttribute("onclick", "createPopUpTribuG(event)");
							div.setAttribute("onmouseout", "resetImage(event)");
							div.setAttribute("onmouseover", "agrandirImage(event)");
							div.setAttribute("data-bs-toggle", "tooltip");
							div.setAttribute("data-bs-placement", "top");
							div.setAttribute("title", "Tribu G " + info_tribuG["name_tribu_t_muable"]);
							div.setAttribute("data-name", e.dataset.name);
							...
							if (
								document.querySelector(".row_content_tribu_pastille_jheo_js") &&
								document
									.querySelector(".row_content_tribu_pastille_jheo_js")
...
function findResto(val, localisation = "") {
	...
	document.querySelector("#elie-restou").innerHTML = `<div class="d-flex justify-content-center">
	...
	fetch(request)
		...
		.then((data) => {
			...
			jsons.length > 1
				? (document.querySelector("#extModalLabel").innerText = jsons.length + " restaurants trouvés")
				: (document.querySelector("#extModalLabel").innerText = jsons.length + " restaurant trouvé");
				...
				for (let json of jsons) {
					...
					const cuisineMonde =
						json.cuisineMonde != "0"
							...
							: "";
					const fastFood =
						json.fastFood != "0" ? `<p><i class="fa-solid fa-burger"></i><span> Fast food </span></p>` : "";
					const creperie =
						json.creperie != "0"
	...
	if (adresse.trim() != "" || inputName.trim() != "") {
		if (
			...
			(document.querySelector("#fetch_golf_tribug_jheo_js") &&
				document.querySelector("#fetch_golf_tribug_jheo_js").classList.contains("active"))
		) {
...
function setUpdateNote(params, id, note, commentaire, id_resto) {
	...
	if (window.location.href.includes("/user/account")) {
		document
			...
			.setAttribute("onclick", "updateNoteTribuG(" + id + ", " + id_resto + ")");
	} else {
		document
			.querySelector(".send_avis_jheo_js")
...
function validatePhoto(id_rubrique, id_gallery, file_path, rubrique) {
	...
	fetch(
		new Request("/" + rubrique + "/validate/photos/" + id_rubrique + "/" + id_gallery, {
			method: "POST",
			headers: {
				Accept: "application/json",
				"Content-Type": "application/json",
...
function rejectPhoto(id_rubrique, id_gallery, file_path, rubrique) {
	...
	fetch(
		new Request("/" + rubrique + "/reject/photos/" + id_rubrique + "/" + id_gallery, {
			method: "POST",
			headers: {
				Accept: "application/json",
				"Content-Type": "application/json",
...
function galleryAll(tribu_t_types) {
	...
	const request = new Request("/user/tribu/photos/" + tribu_t_name_0 + "_publication", {
		method: "GET",
		headers: {
			Accept: "application/json",
			"Content-Type": "application/json",
					...
					for (let photo of photos) {
						if (parseInt(photo.isAlbum) != 1) {
							...
							if (cheminFichier == null) {
								...
								} else if (cheminFichier.indexOf("/photo_imp/") > -1 && count == 2) {
									//TODO date for key and path array as value
...
<div class="content-album-tomm-js d-none" data-status="gal-album-t">
				...
				document.querySelector("#gal-publication-t").addEventListener("click", () => {
					if (
						document.querySelector("#gal-publication-t").getAttribute("data-status-nav") ==
						document.querySelector(".content-publication-tomm-js").getAttribute("data-status")
					) {
						document.querySelector(".content-publication-tomm-js").classList.remove("d-none");
						document.querySelector(".content-agenda-tomm-js").classList.add("d-none");
						document.querySelector(".content-telechargement-tomm-js").classList.add("d-none");
						document.querySelector(".content-album-tomm-js").classList.add("d-none");
						document.querySelector("#gal-publication-t").style = "color: #0101DF";
						document.querySelector("#gal-agenda-t").removeAttribute("style");
						document.querySelector("#gal-telechargement-t").removeAttribute("style");
						document.querySelector("#gal-album-t").removeAttribute("style");
					}
				});
				document.querySelector("#gal-agenda-t").addEventListener("click", () => {
					if (
						document.querySelector("#gal-agenda-t").getAttribute("data-status-nav") ==
						document.querySelector(".content-agenda-tomm-js").getAttribute("data-status")
					) {
						document.querySelector(".content-agenda-tomm-js").classList.remove("d-none");
						document.querySelector(".content-publication-tomm-js").classList.add("d-none");
						document.querySelector(".content-telechargement-tomm-js").classList.add("d-none");
						document.querySelector(".content-album-tomm-js").classList.add("d-none");
						document.querySelector("#gal-agenda-t").style = "color: #0101DF";
						document.querySelector("#gal-publication-t").removeAttribute("style");
						document.querySelector("#gal-telechargement-t").removeAttribute("style");
						document.querySelector("#gal-album-t").removeAttribute("style");
					}
				});
				document.querySelector("#gal-telechargement-t").addEventListener("click", () => {
					if (
						document.querySelector("#gal-telechargement-t").getAttribute("data-status-nav") ==
						document.querySelector(".content-telechargement-tomm-js").getAttribute("data-status")
					) {
						document.querySelector(".content-agenda-tomm-js").classList.add("d-none");
						document.querySelector(".content-publication-tomm-js").classList.add("d-none");
						document.querySelector(".content-telechargement-tomm-js").classList.remove("d-none");
						document.querySelector(".content-album-tomm-js").classList.add("d-none");
						document.querySelector("#gal-telechargement-t").style = "color: #0101DF";
						document.querySelector("#gal-publication-t").removeAttribute("style");
						document.querySelector("#gal-agenda-t").removeAttribute("style");
						document.querySelector("#gal-album-t").removeAttribute("style");
					}
				...
				document.querySelector("#gal-album-t").addEventListener("click", () => {
					if (
						document.querySelector("#gal-album-t").getAttribute("data-status-nav") ==
						document.querySelector(".content-album-tomm-js").getAttribute("data-status")
					) {
						document.querySelector(".content-agenda-tomm-js").classList.add("d-none");
						document.querySelector(".content-publication-tomm-js").classList.add("d-none");
						document.querySelector(".content-telechargement-tomm-js").classList.add("d-none");
						document.querySelector(".content-album-tomm-js").classList.remove("d-none");
...
function setPhotoMessage(btn_photo) {
	if (btn_photo.tagName != "IMG") {
		if (document.querySelector("#img_modal_mess_photo")) {
			document.querySelector("#img_modal_mess_photo").src = btn_photo.querySelector("img").src;
		} else {
			document.querySelector("#img_modal_mess_grp_photo").src = btn_photo.querySelector("img").src;
		}
	} else {
		if (document.querySelector("#img_modal_mess_photo")) {
...
/**
 * @author Faniry
 * cette fonction crypte vos donnée
 * @param {} data  les données à crypter.
 * Cette fonction est dèjà utiliser, donc si vous voluez changer
 * etendez le ou surcharger
 * @returns {string} string  crypté  et encoder en base64
 */
function cryptageJs(data) {
	const key = CryptoJS.enc.Hex.parse("000102030405060708090a0b0c0d0e0f");
	const iv = CryptoJS.enc.Hex.parse("101112131415161718191a1b1c1d1e1f");
	const cipherMessage = CryptoJS.AES.encrypt(data, key, {
		mode: CryptoJS.mode.CBC,
		iv,
	}).toString();
	return btoa(cipherMessage);
}
...
/**
 * @author Elie
 * @constructor Affichage de la description de la publication
 * @used card_publication.html.twig, utilisé dans l'actualité, tribu G et tribu T
 * @param {*} params
 */
function openDetailPublication(params) {
	$("#detailPublication").modal("show");

	let publication = document.querySelector("#" + params);

	// console.log(params.parentElement.parentElement.querySelector(".full_description"));

	let full_description = publication.querySelector(".full_description").textContent;

	let head =
		publication.querySelector(".head-publication-actualite").outerHTML +
		`<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>`;

	let var_react = publication.querySelector(".card-reaction");

	let reaction_and_comment = publication
		.querySelector(".card-reaction")
		.outerHTML.replaceAll("card-reaction d-none", "card-reaction");

	document.querySelector("#detailDescriprion").innerText = full_description;

	document.querySelector("#detailHead").innerHTML = head;

	document.querySelector("#detailReaction").innerHTML = reaction_and_comment;

	if (publication.querySelector(".publication-image")) {
		let image = publication.querySelector(".publication-image").src;

		// for dev
		document.querySelector("#detailImage").src = image;

		//for local
		// document.querySelector("#detailImage").src = image.replaceAll("/public","")
				...
				for (let i = 0; i < jsons.length; i++) {
					if (i === 0) {
						...
						for (let tribut of tribuTs) {
							...
							const logoPath =
								tribut.logo_path != "" ? tribut.logo_path : "/uploads/tribus/avatar_tribu.jpg";
...
*/
function lookupFanIframe(event, useDefaulAction = true, myaction1 = null, myaction2 = null) {
					...
					if (r.status === 200 && r.ok) {
						if (useDefaulAction) {
							...
							r.json().then((jsons) => {
								if (jsons.length > 0) {
									for (let json of jsons) {
										const link = "/api/message/perso_iframe?user_id=" + json.user_id;
									...
									lookContainer.appendChild(anchorElement);
									document.querySelector(".lookup_fan_result_tom_js").classList.toggle("d-none");
									swal({
										title: "Oops...",
										text: "Aucun résultat.",
										icon: "error",
