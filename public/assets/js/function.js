    ...
    fetch(url)
		...
		.then(parsedResult => {
			if( parsedResult ){
				...
				var latlng = L.latLng(45.55401555223028, 3.9946391799233365);

                const json=getDataInLocalStorage("coordFerme") ? JSON.parse(getDataInLocalStorage("coordFerme")) :latlng
...
function addControlPlaceholdersFermeSpec(map) {
    const corners = map._controlCorners
    const l = 'leaflet-'
    const container = map._controlContainer;
...
function  addControlPlaceholdersSpec(map){
    const corners = map._controlCorners
    const l = 'leaflet-'
    const container = map._controlContainer;
...
function addMap(data,dep){
    ...
    if( stations || fermes || resto ){
        ...
        //var latlng = L.latLng(46.227638, 2.213749);
        var latlng = L.latLng(centers[parseInt(dep)].lat, centers[parseInt(dep)].lng);
        //var map = L.map('map', {center: latlng, zoom: getDataInLocalStorage("zoom") ? getDataInLocalStorage("zoom"): 5 , layers: [tiles]});
        const json=getDataInLocalStorage("coordTous") ? JSON.parse(getDataInLocalStorage("coordTous")) :null
        const zoom = json ? (json.zoom ? json.zoom :centers[parseInt(dep)].zoom) : centers[parseInt(dep)].zoom;
