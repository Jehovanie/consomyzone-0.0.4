...
function showModalEditor(isG, isListeInfile = false) {
	...
	if (isListeInfile) {
		if (document.querySelector("#btnValidate")) document.querySelector("#btnValidate").removeAttribute("data-g");
		...
		if (document.querySelector("#btnValidate")) document.querySelector("#btnValidate").dataset.g = isG;
		if (document.querySelector("#btnValidateMessage"))
	...
	let dEvent = agenda ? (agenda.dateStart ? new Date(agenda.dateStart) : new Date()) : new Date();
...
/**
 * alerte pour un restaurant ayant une demande de modification en cours.
 * @constructor
 */
function openSwalRequestWaiting() {
	swal({
		text: "Il y a une demande de modification en cours pour ce restaurant.",
		icon: "info",
	});
}
...
function getListeDemandePartenariat(e) {
	...
	linkActives.forEach((link) => {
		if (link.classList.contains("text-primary")) link.classList.remove("text-primary");
	});
		...
		.then((r) => {
			...
			for (let i = 0; i < r.length; i++) {
				...
				btnDetail.onclick = () => {
					...
					let fnameText =
						r[i].firstname != "" ? r[i].firstname : "Le partenaire n'a pas fourni cette information";
					document.querySelector(".fnameD").textContent = fnameText;
					let lnameText =
						r[i].lastname != "" ? r[i].lastname : "Le partenaire n'a pas fourni cette information";
					document.querySelector(".lnameD").textContent = lnameText;
					let emailProText =
						r[i].emailPro != "" ? r[i].emailPro : "Le partenaire n'a pas fourni cette information";
					document.querySelector(".emailD").textContent = emailProText;
					let telProText =
						r[i].telephone != "" ? r[i].telephone : "Le partenaire n'a pas fourni cette information";
					...
					let commerceText =
						r[i].commerce != "" ? r[i].commerce : "Le partenaire n'a pas fourni cette information";
					document.querySelector(".commerceD").textContent = commerceText;
					let codeapeText =
						r[i].codeape != "" ? r[i].codeape : "Le partenaire n'a pas fourni cette information";
					...
					let siretText =
						r[i].siret || r[i].siret != "" ? r[i].siret : "Le partenaire n'a pas fourni cette information";
...
function sendFeedBackForPartenariat() {
	...
	fetch(request).then((r) => {
		if ((r.status = 200 && r.ok)) {
			swal("Info", `Le mail qui lui explique le refus a été envoyée`, "success").then((value) => {
...
function reorganisePastille() {
	...
	if (document.querySelector(".mainContainerLogoTribu")) {
		...
		if (container.querySelectorAll(".img_nantenaina")) {
			...
			if (imLength > 5) document.querySelector(".iconePlus_nanta_js").classList.remove("d-none");
...
if (document.querySelector(".btn-navright-tribut-tomm-js")) {
	document.querySelector(".btn-navright-tribut-tomm-js").addEventListener("click", () => {
		...
		document.querySelector(".btn-navright-tribut-tomm-js").style.display = "block";
	});
}
