	...
	fetch(request)
		...
		.then((response) => {
			...
			allAgenda.forEach((agenda) => {
				...
				dateStartAgd = dateStartAgd.setHours(
					heureDebut.split(":")[0],
					heureDebut.split(":")[1],
					heureDebut.split(":")[2]
				);
				let dateEndAgd = new Date(end);
				dateEndAgd = dateEndAgd.setHours(
					heureFin.split(":")[0],
					heureFin.split(":")[1],
					heureFin.split(":")[2]
				);
...
function updateAgenda() {
	...
	document.querySelector(".cta_delete_agenda_jheo_js").addEventListener("click", () => {
		...
		if (!agendaID) {
			showAlertMessageFlash(
				"Impossible de supprimer cette agenda parce que ce n'est pas vous qu'il a crée.",
				"danger"
			);
		} else {
...
function sendNewAgenda(agenda) {
	const param = {
		...
		resto: agenda.resto,
		resto:
			document.querySelector(".restoEvent_jheo_js").value !== ""
				? document.querySelector(".restoEvent_jheo_js").value
				: null,
		confidentiality: 1,
		fileType: null,
...
function bindActionSearchResto(motCles) {
	...
	createChargement(
		document.querySelector(".content_chargement_jheo_js"),
	...
	let titleModal =
		motCles === "RESTO_PASTILLE"
			? "Liste des restaurants pastillés dans vos tribu Tribu T."
			: "Résultat de restaurant trouver";
			...
			.then((response) => {
				...
				if (results.length > 0) {
					results.forEach((resto, index) => {
						createLinkRestoPastile(
							index,
							{ id: resto.id_resto, name: resto.name, adress: resto.adress },
							true
						);
					});
				} else {
...
function createLinkRestoPastile(index, resto, isValid = true) {
	if (isValid) {
		...
		tr.innerHTML = `
          ...
          <td><button type="button" class="btn btn-outline-info" onclick="setRestoAgenda({ name:'${
				resto.name
			}',adress:'${resto.adress.toLowerCase()}'} ,this)">Choisissez</button></td>
        `;
...
function getEtabByCodeinse(element) {
	...
	let request = new Request(
		`/api/user/agenda/arrondissement/restaurant/codeinse?id_dep=${id_dep}&codinsee=${codinsee}`,
		{
			method: "GET",
			headers: {
				Accept: "application/json",
				...
				results.forEach((etablissement, index) => {
					...
					if (etablissement.tribu) {
						...
						if (etab === "golf") {
							...
							if (etablissement[0])
								status =
									etablissement[0].afaire === 1
										? "à faire"
										: etablissement[0].fait === 1
										? "fait"
										: "Pas encore pastillé";
							document.querySelector("#tableEtabCMZ > tbody").appendChild(
								generateTableForEtab(
...
function generateTableForEtab(index, etab, isValid = true, etabObject = null) {
	if (isValid) {
        ...
        if (etab.tribu && etab.tribu != null) {
			...
			if (document.querySelector(".forLogo")) document.querySelector(".forLogo").remove();

			if (document.querySelector(".forTribu")) document.querySelector(".forTribu").remove();
		}
		...
		if (etab.status) {
			...
			if (document.querySelector(".forGolfStatus")) document.querySelector(".forGolfStatus").remove();
		}
		...
		btnDetails.setAttribute(
			"onclick",
			`showEtabDetail(event, '${etab.departement}', '${etab.dep}', '${etab.id_etab}')`
		);
		...
		btnCheck.setAttribute(
			"onclick",
			`setNameOrAdresseForEtab({ name:"${etab.nom}",adress:"${etab.adresse.toLowerCase()}"} ,this)`
		);
...
function makeLoading() {
	...
	createChargement(
		document.querySelector(".content_chargement_nanta_js"),
		(c = "chargement_content content_chargement_nanta_js")
	);
...
function getObjectForNewAgenda(e) {
	...
	if (
		document.querySelector(".preview_image_nanta_js") &&
		!document.querySelector(".preview_image_nanta_js").classList.contains("d-none")
	) {
	...
	try {
		checkRequiredInput(
			...
			"veuillez renseigner le titre de votre événement",
		...
		checkRequiredInput(
			document.querySelector("#eventStart"),
			"veuillez renseigner la date de début de votre événement",
			containerMessageRequiredAgd
		);
		checkRequiredInput(
			document.querySelector("#eventEnd"),
			"veuillez renseigner la date de fin de votre événement",
			containerMessageRequiredAgd
		);
		checkRequiredInput(
			document.querySelector("#timeStart"),
			"veuillez renseigner l'heure de début de votre événement",
			containerMessageRequiredAgd
		);
		checkRequiredInput(
			document.querySelector("#timeEnd"),
...
function tableActiveFilterPartisant(e) {
	const allTypeActive = [
		"list_partisant_tribuG_jheo_js",
		"list_partisant_tribuT_jheo_js",
		"email_non_inscrit",
		"invitation_story",
		"list_partisant_emailing_jheo_js",
	];
	...
	if (e.classList.contains("redirect_inscrit_g_tomm_js")) {
		document.querySelector("li > .email_non_inscrit").classList.add("active");
	}

	if (document.querySelector(".btn-close-partisans-tribuT")) {
		document.querySelector(".btn-close-partisans-tribuT").click();
	}
...
if (document.querySelector("#list-tribu-g-partage-agenda")) {
	...
	$("#list-tribu-g-partage-agenda").DataTable({
		language: {
			url: "//cdn.datatables.net/plug-ins/1.13.4/i18n/fr-FR.json",
			search: "Recherche global",
			emptyTable: `Aucun fan à part vous dans ce tribu <a class="btn btn-success email_non_inscrit redirect_inscrit_g_tomm_js" data-target="agenda-non-inscrit" onclick="tableActiveFilterPartisant(this)">inviter des fans</a>`,
		},
	});
}
...
function showPartisanAgenda(tribu_t_name) {
	...
	document.querySelector(
		"#list-partisans-tribu-t-partage-agenda"
	).innerHTML = `<table id="list-partisans-tribuT" class="table table-striped">
                        <thead>
                            <tr>
				...
				jsons[0].forEach((json) => {
					if (jsons["curent_user"] != json.id) {
						...
						if (document.querySelector(`.btn-close-partisans-${tribu_t_name}`)) {
							document
								.querySelector(`.btn-close-partisans-${tribu_t_name}`)
...
function handleChange(elment, isG) {
	if (isG) {
		if (elment.checked) {
			...
			for (var i = 0; i < checkboxes.length; i++) {
				if (!checkboxes[i].checked) allChekecd = false;
			}
			if (allChekecd) {
			...
			let hasChecked = false;
			for (var i = 0; i < checkboxes.length; i++) {
				if (checkboxes[i].checked) hasChecked = true;
			}
...
function putInputOnDataTableHeader(selector, colIdx, api) {
	...
	var cell = $(selector + " .filters th").eq($(api.column(colIdx).header()).index());
	var title = $(cell).text();
