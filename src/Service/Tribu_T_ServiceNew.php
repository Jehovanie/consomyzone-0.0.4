...
class Tribu_T_ServiceNew extends PDOConnexionService
{
   ...
   * @param object $user : l'utilisateur connecté
   */
  public function addTribuTOwned(
    $user,
    $name_tribu_t_muable,
    $description,
    $logo_path,
  ...
  public function checkIfCurrentGolfPastilled($tableNameExtension, int $idGolf, $isPastilled)
  {
    ...
    $statement = $this->getPDO()->prepare("SELECT id FROM $tableNameExtension WHERE extensionId = $idGolf AND isPastilled = $isPastilled");
  ...
  public function getGolfPastilles($tableGolf, $tableComment)
  {
    ...
    $sql = "SELECT * FROM (SELECT t1.id , t2.id as id_golf_comment, t1.extensionId  as id_golf ,t1.denomination_f, 
                          t1.isPastilled, t2.extensionId as id_golfer, t2.userId as id_user,t2.note,t2.commentaire,
                          GROUP_CONCAT(t2.userId) as All_user ,GROUP_CONCAT(t2.commentaire) as All_com,FORMAT(AVG(t2.note),2) as globalNote, COUNT(t2.extensionId) as nbrAvis ,
                          GROUP_CONCAT(t2.id) as All_id_r_com
                          FROM  $tableGolf as t1 left join $tableComment  as t2 on t1.extensionId=t2.extensionId where  t1.isPastilled IS TRUE GROUP BY t1.id ) as tableGolfCom  
          INNER JOIN golffrance ON  tableGolfCom.id_golf=golffrance.id";
    $stmt = $this->getPDO()->prepare($sql);
